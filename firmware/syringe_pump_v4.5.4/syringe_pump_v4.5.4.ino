/*
 Version of code: V4.5.4 - USB Serial Only
 Version of PCB board: V3.0
 
 KEY FEATURES (v4.5.4):
 - Microstepping feature added
*/
...
// Stepper driver controllers for command communication
TMC2209Stepper driver_a(&TMCSerial, R_SENSE, DRIVER_ADDR_A);
TMC2209Stepper driver_b(&TMCSerial, R_SENSE, DRIVER_ADDR_B);
TMC2209Stepper driver_c(&TMCSerial, R_SENSE, DRIVER_ADDR_C);
TMC2209Stepper driver_d(&TMCSerial, R_SENSE, DRIVER_ADDR_D);
...
void setMicrostepping(int motor_index, int microstep_value) {
    ...
    if (microstep_value != 0 && (microstep_value < 2 || microstep_value > 256 || (microstep_value & (microstep_value - 1)) != 0)) {return;}


    switch(motor_index) 
    {
        case 0: { driver_a.microsteps(microstep_value);delay(50);break;}
...
void initStepperDrivers() 
{
    ...
    driver_a.begin();// Initialize driver
    // Muy importante para usar UART en vez de la config por pin
    driver_a.pdn_disable(true);
    driver_a.mstep_reg_select(true);
 
    driver_a.toff(5);
    driver_a.rms_current(600);
    driver_a.microsteps(motors[0].microstep_type);   // default value
    driver_a.I_scale_analog(false);

    driver_b.begin();// Initialize driver
    // Muy importante para usar UART en vez de la config por pin
    driver_b.pdn_disable(true);
    driver_b.mstep_reg_select(true);
 
    driver_b.toff(5);
    driver_b.rms_current(600);
    driver_b.microsteps(motors[1].microstep_type);   // default value
    driver_b.I_scale_analog(false);

    driver_c.begin();// Initialize driver
    // Muy importante para usar UART en vez de la config por pin
    driver_c.pdn_disable(true);
    driver_c.mstep_reg_select(true);
 
    driver_c.toff(5);
    driver_c.rms_current(600);
    driver_c.microsteps(motors[2].microstep_type);   // default value
    driver_c.I_scale_analog(false);

    driver_d.begin();// Initialize driver
    // Muy importante para usar UART en vez de la config por pin
    driver_d.pdn_disable(true);
    driver_d.mstep_reg_select(true);

    driver_d.toff(5);
    driver_d.rms_current(600);
    driver_d.microsteps(motors[3].microstep_type);   // default value
    driver_d.I_scale_analog(false);
